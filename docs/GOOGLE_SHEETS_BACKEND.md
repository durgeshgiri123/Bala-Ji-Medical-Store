# Google Sheets Backend Code — Bala Ji Medical Store

This document explains how to connect the **Bala Ji Medical Store** Inventory & Billing Management Web Application to Google Sheets using **Google Apps Script**. The application uses Google Sheets as the database and Apps Script as the API backend, so no Node.js, PHP, Python, MySQL, MongoDB, or Firebase backend is required.

## 1. Required Google Sheet Tabs

Create one Google Spreadsheet and add these six sheets with the exact names and column headers below.

### Employees

| EmployeeID | Name | Email | Phone | Address | JoiningDate | Role | Password | Status |
|---|---|---|---|---|---|---|---|---|

### Medicines

| MedicineID | MedicineName | Category | Company | BatchNo | MFGDate | ExpiryDate | StockQuantity |
|---|---|---|---|---|---|---|---|

> Inventory intentionally does **not** include price, purchase cost, or MRP fields. Stock starts from `0` when a medicine is added.

### StockHistory

| Date | MedicineID | MedicineName | PreviousStock | AddedQuantity | NewStock | Employee |
|---|---|---|---|---|---|---|
...
| InvoiceNo | Date | CustomerName | Mobile | Email | TotalAmount | EmployeeName |
|---|---|---|---|---|---|---|

### BillItems

| InvoiceNo | MedicineName | Quantity | Price | Total |
|---|---|---|---|---|
...
3. Paste the code from `backend/google-apps-script.gs` or from the code block below.
4. Replace `YOUR_SPREADSHEET_ID_HERE` with your spreadsheet ID from the Sheet URL.
5. Click **Deploy → New deployment → Web app**.
6. Set **Execute as** to `Me`.
7. Set **Who has access** to `Anyone` or your required staff access level.
8. Copy the Web App URL.
9. Paste that URL into `CONFIG.APPS_SCRIPT_URL` in `js/app.js`.

## 3. API Actions Used by the Frontend

The frontend sends a `POST` request with:

- `action`: API method name such as `login`, `getMedicines`, `addMedicine`, `saveBill`, or `getReport`.
- `data`: JSON string payload for that action.

Core actions include:

- Authentication: `login`, `forgotPassword`
- Employees: `getEmployees`, `addEmployee`, `updateEmployee`, `toggleEmployee`
- Medicines: `getMedicines`, `addMedicine`, `updateMedicine`, `updateStock`
- Billing: `saveBill`, `getBills`, `getBillItems`, `sendEmail`
- Reports: `getDashboard`, `getReport`

## 4. Google Apps Script Source Code
