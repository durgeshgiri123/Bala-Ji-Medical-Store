...
prescriptions.html  Doctor prescription module
...
### Employee Medicine Management

Employees, Managers, and Admins can now add medicines, edit medicine details, update categories, update remarks, and add incoming stock. Only Admin users can permanently delete medicines.

### Duplicate Validation

Employee creation checks duplicate username, phone, and email. If a duplicate is found, the API returns:

```text
Employee already exists.
```

Medicine creation checks duplicate medicine name + category. If a duplicate is found, the API returns:

```text
Medicine already exists.
```

Admin users can bypass duplicate checks by selecting the relevant **Allow Duplicate** checkbox.

### Optional GST

Billing now includes an **Apply GST** checkbox. If checked, the configured GST rate is applied. If unchecked, GST is `0`. Invoices show subtotal, GST amount, and grand total.

### Payment Before Invoice

Invoices can be generated only after payment confirmation:

1. Select medicines.
2. Review subtotal/GST/grand total.
3. Choose **UPI Payment** or **Cash Payment**.
4. For UPI, scan the dynamic QR code for `6387499392@ybl` and click **I Have Paid**.
5. For cash, check **Cash Payment Received**.
6. Generate the invoice.

The payment method and paid status are saved in the `Bills` sheet.

### Stock Validation and Automatic Stock Reduction

Before billing, the system checks available stock. If requested quantity is higher than stock, billing is blocked with:

```text
Insufficient stock available.
```

After a successful invoice, sold quantities are automatically subtracted from inventory.

### Doctor Role and Prescriptions

A new `Doctor` role is available. Doctors can create prescriptions, add consultation notes, recommend medicines, print prescriptions, and save them to the `Prescriptions` sheet.

The new `Prescriptions` sheet columns are:

```text
PrescriptionID, DoctorID, DoctorName, CustomerID, CustomerName, Medicines, Notes, Date
```

### Audit Logs

Audit logs now use these columns:

```text
LogID, UserID, UserRole, Action, Details, DateTime
```

Tracked actions include login, logout, medicine changes, employee changes, invoice generation, payment confirmation, and prescription creation. Admin users can view recent audit logs on the dashboard.
