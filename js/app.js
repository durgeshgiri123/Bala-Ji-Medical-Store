...
const state = { medicines: [], employees: [], customers: [], bills: [], billItems: [], prescriptions: [], auditLogs: [], lastInvoice: null };
...
async function loadInitialData() {
  if (isApiConfigured()) {
    ...
    state.prescriptions = readStore('bjms_prescriptions');
    state.auditLogs = readStore('bjms_audit_logs');
  }
}
...
function logout() { if (isApiConfigured() && getSession()) api('logout', {}).catch(() => {}); clearSession(); location.href = 'login.html'; }
...
function initAdmin() {
  ...
  document.getElementById('activityLog').innerHTML = state.auditLogs?.length ? `<div class="table-responsive"><table class="table table-sm"><thead><tr><th>User</th><th>Role</th><th>Action</th><th>Details</th><th>Date</th></tr></thead><tbody>${state.auditLogs.map((log) => `<tr><td>${escapeHtml(log.UserID || '')}</td><td>${escapeHtml(log.UserRole || '')}</td><td>${escapeHtml(log.Action || '')}</td><td>${escapeHtml(log.Details || '')}</td><td>${escapeHtml(log.DateTime || '')}</td></tr>`).join('')}</tbody></table></div>` : '<ul><li>No audit log entries yet.</li></ul>';
...
async function saveMedicine(event) {
  ...
  try {
    if (isApiConfigured()) await api('saveMedicine', data);
      ...
      const duplicate = state.medicines.find((m) => m.MedicineID !== data.MedicineID && m.MedicineName.toLowerCase() === data.MedicineName.trim().toLowerCase() && m.Category.toLowerCase() === data.Category.trim().toLowerCase());
      if (duplicate && !(hasRole(['Admin']) && data.AllowDuplicateMedicine === 'on')) throw new Error('Medicine already exists.');
...
function initBilling() { renderBillItems(); renderBillsTable(); document.getElementById('addBillItem')?.addEventListener('click', () => addBillItemRow()); document.getElementById('billForm')?.addEventListener('submit', saveBill); document.getElementById('printInvoice')?.addEventListener('click', () => window.print()); document.getElementById('sendWhatsapp')?.addEventListener('click', sendWhatsApp); document.getElementById('sendEmail')?.addEventListener('click', sendInvoiceEmail); document.getElementById('exportBillPdf')?.addEventListener('click', () => window.print()); document.getElementById('applyGst')?.addEventListener('change', calculateBill); document.querySelectorAll('input[name="PaymentMethod"]').forEach((el) => el.addEventListener('change', updatePaymentScreen)); document.getElementById('cashReceived')?.addEventListener('change', updatePaymentScreen); document.getElementById('upiPaid')?.addEventListener('click', () => { document.getElementById('upiConfirmed').value = 'yes'; updatePaymentScreen(); }); addBillItemRow(); updatePaymentScreen(); }
...
function calculateBill() { let subtotal = 0; document.querySelectorAll('#billItems tr').forEach((row) => { const qty = Number(row.querySelector('.bill-qty').value || 0); const price = Number(row.querySelector('.bill-price').value || 0); const total = qty * price; row.querySelector('.bill-line-total').textContent = money(total); subtotal += total; }); const applyGst = document.getElementById('applyGst')?.checked || false; const gst = applyGst ? subtotal * CONFIG.GST_RATE : 0; document.getElementById('billSubtotal').textContent = money(subtotal); document.getElementById('billGst').textContent = money(gst); document.getElementById('billGrandTotal').textContent = money(subtotal + gst); updatePaymentScreen(subtotal + gst); return { subtotal, gst, grandTotal: subtotal + gst, applyGst }; }
function updatePaymentScreen(amount) { const totals = typeof amount === 'number' ? { grandTotal: amount } : calculateBill(); const method = document.querySelector('input[name="PaymentMethod"]:checked')?.value || 'UPI'; document.getElementById('upiPanel')?.classList.toggle('d-none', method !== 'UPI'); document.getElementById('cashPanel')?.classList.toggle('d-none', method !== 'Cash'); document.querySelectorAll('[data-payable]').forEach((el) => { el.textContent = money(totals.grandTotal); }); const upiText = `upi://pay?pa=6387499392@ybl&pn=${encodeURIComponent(CONFIG.SHOP.name)}&am=${totals.grandTotal.toFixed(2)}&cu=INR`; const qr = document.getElementById('upiQr'); if (qr) qr.src = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(upiText)}`; }
function collectBillItems() { return [...document.querySelectorAll('#billItems tr')].map((row) => { const id = row.querySelector('.bill-med').value; const med = state.medicines.find((m) => m.MedicineID === id); return { MedicineID: id, MedicineName: med?.MedicineName || id, Quantity: Number(row.querySelector('.bill-qty').value), Price: Number(row.querySelector('.bill-price').value) }; }).filter((i) => i.MedicineID && i.Quantity > 0); }
function validateStock(items) { return items.every((item) => { const med = state.medicines.find((m) => m.MedicineID === item.MedicineID); return med && item.Quantity <= Number(med.AvailableQuantity || 0); }); }
async function saveBill(event) { event.preventDefault(); const form = event.currentTarget; if (!form.checkValidity()) return form.reportValidity(); const totals = calculateBill(); const items = collectBillItems(); if (!items.length) return notify('Add at least one medicine.', 'danger'); if (!validateStock(items)) return notify('Insufficient stock available.', 'danger'); const paymentMethod = document.querySelector('input[name="PaymentMethod"]:checked')?.value || 'UPI'; const paymentConfirmed = paymentMethod === 'UPI' ? document.getElementById('upiConfirmed')?.value === 'yes' : document.getElementById('cashReceived')?.checked; if (!paymentConfirmed) return notify('Confirm payment before generating invoice.', 'danger'); const bill = { BillID: `BILL-${Date.now()}`, CustomerName: form.CustomerName.value.trim(), Phone: form.Phone.value.trim(), Email: form.Email.value.trim(), TotalAmount: totals.grandTotal, Subtotal: totals.subtotal, GSTAmount: totals.gst, ApplyGST: totals.applyGst, PaymentMethod: paymentMethod, PaymentConfirmed: true, PaymentStatus: 'Paid', Date: new Date().toLocaleString(), CreatedBy: getSession()?.name || 'Demo', items }; try { if (isApiConfigured()) { const result = await api('createBill', bill); bill.BillID = result.BillID || bill.BillID; } else { items.forEach((item) => { const med = state.medicines.find((m) => m.MedicineID === item.MedicineID); med.AvailableQuantity = Number(med.AvailableQuantity || 0) - item.Quantity; }); state.bills.unshift(bill); writeStore('bjms_bills', state.bills); writeStore('bjms_medicines', state.medicines); } state.lastInvoice = bill; renderInvoice(bill); renderBillsTable(); ['sendWhatsapp','sendEmail','exportBillPdf'].forEach((id) => { document.getElementById(id).disabled = false; }); notify('Payment confirmed and invoice generated successfully.'); } catch (error) { notify(error.message, 'danger'); } }
function renderInvoice(bill) { document.getElementById('invoicePreview').innerHTML = `<div class="invoice-header"><img src="assets/logo.svg" alt="Logo"><div><h2 class="h5 mb-0">${CONFIG.SHOP.name}</h2><small>${CONFIG.SHOP.address}</small><br><small>${CONFIG.SHOP.phone} · ${CONFIG.SHOP.email}</small></div></div><div class="row small"><div class="col-6"><strong>Invoice:</strong> ${bill.BillID}<br><strong>Date:</strong> ${bill.Date}<br><strong>Employee:</strong> ${escapeHtml(bill.CreatedBy || '')}<br><strong>Payment:</strong> ${escapeHtml(bill.PaymentMethod || '')} (${escapeHtml(bill.PaymentStatus || 'Paid')})</div><div class="col-6"><strong>Customer:</strong> ${escapeHtml(bill.CustomerName)}<br><strong>Phone:</strong> ${escapeHtml(bill.Phone)}<br><strong>Email:</strong> ${escapeHtml(bill.Email || '-')}</div></div><table class="table table-sm mt-3"><thead><tr><th>Medicine</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead><tbody>${bill.items.map((i) => { const med = state.medicines.find((m) => m.MedicineID === i.MedicineID); return `<tr><td>${escapeHtml(med?.MedicineName || i.MedicineID)}</td><td>${i.Quantity}</td><td>${money(i.Price)}</td><td>${money(i.Price * i.Quantity)}</td></tr>`; }).join('')}</tbody></table><div class="text-end"><p class="mb-1">Subtotal: <strong>${money(bill.Subtotal)}</strong></p><p class="mb-1">GST: <strong>${money(bill.GSTAmount)}</strong></p><h3 class="h5">Grand Total: ${money(bill.TotalAmount)}</h3></div>`; }
...
async function saveEmployee(event) { event.preventDefault(); const form = event.currentTarget; const data = Object.fromEntries(new FormData(form)); if (!data.EmployeeID && !data.Password) return notify('Password is required for a new employee.', 'danger'); data.PasswordHash = data.Password ? await sha256(data.Password) : ''; delete data.Password; try { if (isApiConfigured()) await api('saveEmployee', data); else { const index = state.employees.findIndex((e) => e.EmployeeID === data.EmployeeID); const duplicate = state.employees.find((e) => e.EmployeeID !== data.EmployeeID && (e.Username === data.Username || (data.Phone && e.Phone === data.Phone) || (data.Email && String(e.Email).toLowerCase() === data.Email.toLowerCase()))); if (duplicate && !(hasRole(['Admin']) && data.AllowDuplicate === 'on')) throw new Error('Employee already exists.'); const row = { ...data, EmployeeID: data.EmployeeID || `EMP-${Date.now()}`, PasswordHash: data.PasswordHash || state.employees[index]?.PasswordHash, CreatedDate: today() }; index >= 0 ? state.employees.splice(index, 1, row) : state.employees.push(row); writeStore('bjms_employees', state.employees); } form.reset(); await loadInitialData(); renderEmployees(); notify('Employee saved.'); } catch (error) { notify(error.message, 'danger'); } }
...
function initPrescriptions() {
  renderPrescriptionCustomerOptions();
  document.getElementById('prescriptionForm')?.addEventListener('submit', savePrescription);
  document.getElementById('printPrescription')?.addEventListener('click', () => window.print());
  document.getElementById('sendPrescriptionPdf')?.addEventListener('click', () => notify('Use Print Prescription and choose Save as PDF, then send it to the customer.', 'info'));
  renderPrescriptions();
}
function renderPrescriptionCustomerOptions() {
  const select = document.getElementById('prescriptionCustomer');
  if (!select) return;
  select.innerHTML = '<option value="">Walk-in / New Customer</option>' + state.customers.map((c) => `<option value="${c.CustomerID}">${escapeHtml(c.Name)} (${escapeHtml(c.Phone)})</option>`).join('');
}
async function savePrescription(event) {
  event.preventDefault();
  const form = event.currentTarget;
  const data = Object.fromEntries(new FormData(form));
  const customer = state.customers.find((c) => c.CustomerID === data.CustomerID);
  const row = { PrescriptionID: `RX-${Date.now()}`, CustomerID: data.CustomerID || '', CustomerName: customer?.Name || data.CustomerName || 'Walk-in Customer', Medicines: data.Medicines, Notes: data.Notes, Date: new Date().toLocaleString(), DoctorName: getSession()?.name || 'Doctor' };
  try {
    if (isApiConfigured()) await api('savePrescription', row);
    else { state.prescriptions.unshift(row); writeStore('bjms_prescriptions', state.prescriptions); }
    form.reset(); renderPrescriptions(); notify('Prescription saved.');
  } catch (error) { notify(error.message, 'danger'); }
}
function renderPrescriptions() {
  const area = document.getElementById('prescriptionList');
  if (!area) return;
  area.innerHTML = (state.prescriptions || []).map((rx) => `<div class="card border-0 shadow-sm mb-3"><div class="card-body"><div class="d-flex justify-content-between"><strong>${escapeHtml(rx.PrescriptionID)}</strong><span>${escapeHtml(rx.Date)}</span></div><p class="mb-1"><strong>Patient:</strong> ${escapeHtml(rx.CustomerName)}</p><p class="mb-1"><strong>Medicines:</strong> ${escapeHtml(rx.Medicines)}</p><p class="mb-0"><strong>Notes:</strong> ${escapeHtml(rx.Notes)}</p></div></div>`).join('') || '<div class="alert alert-info">No prescriptions yet.</div>';
}
